...
fp, fn, tp, tn = (np.count_nonzero(c[c < 0]) / n * 100, np.count_nonzero(c[c > 0]) / n * 100,
                  np.count_nonzero(np.bitwise_and(a == b, a == 1)) / n * 100,
                  np.count_nonzero(np.bitwise_and(a == b, a == 0)) / n * 100)
precision = (tp / (tp + fp))
recall = (tp / (tp + fn))
